...
public class ContainerCloudSimExample1 {
    ...
    public static List<Cloudlet> createContainerCloudletList(int brokerId, int numberOfCloudlets) {
        ...
        // Get path to workload folder
        String inputFolderName = "src/main/resources/workload/planetlab/";
        java.io.File planetLabFolder = new java.io.File(inputFolderName);
        
        if (!planetLabFolder.exists()) {
            System.out.println("Error: PlanetLab folder not found at: " + planetLabFolder.getAbsolutePath());
            return cloudletList;
        }
        ...
        java.io.File[] dateFolders = planetLabFolder.listFiles();
        if (dateFolders == null || dateFolders.length == 0) {
            System.out.println("Error: No date folders found in planetlab directory");
            return cloudletList;
        }
        ...
        for (java.io.File dateFolder : dateFolders) {
            ...
            java.io.File[] files = dateFolder.listFiles();
            ...
            for (java.io.File file : files) {
                ...
                try {
                    ...
                    System.out.println("Created cloudlet from file: " + file.getName());
                    
                } catch (Exception e) {
                    System.out.println("Error creating cloudlet from file: " + file.getName());
                    e.printStackTrace();
                }
            }
        }
        ...
        if (cloudletList.isEmpty()) {
            System.out.println("Warning: No cloudlets were created. Using default UtilizationModel...");
            // Create at least one cloudlet with default settings
            Cloudlet cloudlet = new Cloudlet(
                IDs.pollId(Cloudlet.class), 
                ConstantsExamples.CLOUDLET_LENGTH, 
                1,
